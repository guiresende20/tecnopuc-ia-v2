...
> **Segurança:** as variáveis `NEXT_PUBLIC_SUPABASE_URL` e `NEXT_PUBLIC_SUPABASE_ANON_KEY` foram removidas intencionalmente. O browser nunca acessa o Supabase diretamente — todas as queries passam por API Routes usando a `service_role` key no servidor. Se no futuro for necessário acesso client-side (ex: Realtime), reintroduza as variáveis **e** habilite RLS com policies adequadas antes.
